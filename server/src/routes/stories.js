...
router.get(
  '/:id/characters/:characterId/greetings',
  asyncHandler(async (req, res) => {
    const { id: storyId, characterId } = req.params;
    ...
    if (
      characterCard.data?.alternate_greetings &&
      Array.isArray(characterCard.data.alternate_greetings)
    ) {
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        if (chunk.finished && imagePreserver) {
          ...
          const { finalContent, imagesRestored, imagesPreserved, imagesMissing } =
            imagePreserver.restoreImages(fullContent, { appendMissing: appendMissingImages });
