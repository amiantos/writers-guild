...
router.post('/:id/characters', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (isAutoGeneratedTitle(story.title)) {
      // Get all character names in the story (including the newly added one)
      const storyCharacters = await storage.listStoryCharacters(req.params.id);
      const characterNames = [];
      for (const char of storyCharacters) {
        try {
          const charCard = await storage.getCharacter(char.id);
          if (charCard.data?.name) {
            characterNames.push(charCard.data.name);
          }
        } catch (error) {
          console.error(`Failed to load character ${char.id} for title:`, error);
        }
      }
...
router.delete('/:id/characters/:characterId', asyncHandler(async (req, res) => {
  ...
  if (shouldUpdateTitle) {
    ...
    const remainingCharacters = await storage.listStoryCharacters(storyId);
    const characterNames = [];
